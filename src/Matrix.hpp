...
namespace linear {
    ...
    concept CompatibleForAddition =
        (R1 == R2 or R1 == DynamicSize or R2 == DynamicSize) and (C1 == C2 or C1 == DynamicSize or C2 == DynamicSize);

    template <ptrdiff_t C1, ptrdiff_t R2>
    concept CompatibleForMultiplication = (C1 == R2) or (C1 == DynamicSize) or (R2 == DynamicSize);
    ...
    class Matrix {
      ...
      public:
        ...
        template <typename... InScalar>
            requires(Rows == 1 and Cols == DynamicSize and (std::convertible_to<InScalar, Scalar> and ...))
        ...
        constexpr Matrix(const std::initializer_list<InScalar>&... args)
            ...
            if constexpr (isDynamic) {
                ...
                assert(m_rows == static_cast<size_t>(Rows) and m_cols == static_cast<size_t>(Cols));
            }
        ...
        template <Numeric InScalar, ptrdiff_t InRows, ptrdiff_t InCols>
            requires(std::convertible_to<InScalar, Scalar> and CompatibleForAddition<Rows, Cols, InRows, InCols>)
        ...
        template <Numeric InScalar, ptrdiff_t InRows, ptrdiff_t InCols>
            requires(std::same_as<Scalar, InScalar> and
                     ...
                     std::same_as<std::integral_constant<ptrdiff_t, Cols>, std::integral_constant<ptrdiff_t, InCols>>)
        ...
        // Fill with value
        constexpr void fill(Scalar value) noexcept { std::fill(m_container.begin(), m_container.end(), value); }
        ...
        // Set submatrix (write block into current matrix)
        template <Numeric InScalar, ptrdiff_t InRows, ptrdiff_t InCols>
            requires(std::convertible_to<InScalar, Scalar>)
        constexpr void
        set_submatrix(size_t start_row, size_t start_col, const Matrix<InScalar, InRows, InCols>& submatrix) {
            assert(start_row + submatrix.rows() <= m_rows);
            assert(start_col + submatrix.cols() <= m_cols);

            for (size_t i = 0; i < submatrix.rows(); ++i) {
                for (size_t j = 0; j < submatrix.cols(); ++j) {
                    (*this)(start_row + i, start_col + j) = static_cast<Scalar>(submatrix(i, j));
                }
            }
        }
        ...
        constexpr auto dot(const Matrix<InScalar, InRows, InCols>& rhs) const {
            ...
            for (size_t i = 0; i < m_cols; ++i) {
                result += static_cast<ResultType>((*this)(0, i)) * static_cast<ResultType>(rhs(0, i));
            }
        ...
        constexpr Matrix<Scalar, DynamicSize, DynamicSize> transposed() const
            requires(isDynamic)
        {
        ...
        constexpr void transpose()
            requires(isRowsDynamic and isColsDynamic)
        {
            *this = transposed();
        }

        // Forward substitution: solve Lx = b where L is lower triangular with unit
        // diagonal
        constexpr Matrix<Scalar, DynamicSize, DynamicSize>
        forward_substitution(const Matrix<Scalar, DynamicSize, DynamicSize>& b) const {
            ...
            for (size_t col = 0; col < b.cols(); ++col) {
                for (size_t i = 0; i < m_rows; ++i) {
                    ...
                    x(i, col) = sum / (*this)(i, i); // Divide by diagonal element
                }
            }
        ...
        // Check if matrix is symmetric (static matrices)
            ...
            requires(SquareMatrix<Rows, Cols> and FloatingPoint<Scalar>)
        {
            ...
            for (size_t i = 0; i < n; ++i) {
                for (size_t j = i + 1; j < n; ++j) {
                    if (std::abs((*this)(i, j) - (*this)(j, i)) > eps) {
                        return false;
                    }
                }
            }
        ...
        // Check if matrix is symmetric (dynamic matrices)
        constexpr bool isSymmetric() const
            requires(DynamicMatrix<Rows, Cols> and FloatingPoint<Scalar>)
        {
            assert(m_rows == m_cols);
        ...
        // Check if matrix is diagonal (static matrices)
        ...
        // Check if matrix is diagonal (dynamic matrices)
        constexpr bool isDiagonal() const
            requires(DynamicMatrix<Rows, Cols> and FloatingPoint<Scalar>)
        {
            assert(m_rows == m_cols);
        ...
        // Check if matrix is upper triangular (static matrices)
        ...
        // Check if matrix is upper triangular (dynamic matrices)
        constexpr bool isUpperTriangular() const
            requires(DynamicMatrix<Rows, Cols> and FloatingPoint<Scalar>)
        {
            assert(m_rows == m_cols);
        ...
        // Check if matrix is lower triangular (static matrices)
        ...
        // Check if matrix is lower triangular (dynamic matrices)
        constexpr bool isLowerTriangular() const
            requires(DynamicMatrix<Rows, Cols> and FloatingPoint<Scalar>)
        {
            assert(m_rows == m_cols);
        ...
        // Check if matrix is orthogonal (Q^T * Q = I) (static matrices)
            ...
            requires(SquareMatrix<Rows, Cols> and FloatingPoint<Scalar>)
        {
            ...
            // Compute Q^T * Q manually and compare with identity
            for (size_t i = 0; i < n; ++i) {
                for (size_t j = 0; j < n; ++j) {
                    ...
                    if (std::abs(dot_product - expected) > eps) {
                        return false;
                    }
                }
            }
        ...
        // Check if matrix has orthonormal columns (Q^T * Q = I) (dynamic matrices)
        // Works for both square and rectangular (m >= n) matrices
        constexpr bool isOrthogonal() const
            requires(DynamicMatrix<Rows, Cols> and FloatingPoint<Scalar>)
        {
            // For m×n matrix, check if columns are orthonormal (Q^T * Q = I_n)
            const size_t m = m_rows;
            const size_t n = m_cols;

            // Compute Q^T * Q and compare with n×n identity
            for (size_t i = 0; i < n; ++i) {
                for (size_t j = 0; j < n; ++j) {
                    Scalar dot_product = Scalar(0);
                    ...
                    Scalar expected = (i == j) ? Scalar(1) : Scalar(0);
                    if (std::abs(dot_product - expected) > std::numeric_limits<Scalar>::epsilon()) {
                        return false;
                    }
                }
            }
        ...
        constexpr std::pair<Matrix<Scalar, DynamicSize, DynamicSize>, Matrix<Scalar, DynamicSize, DynamicSize>>
        qr() const
            requires(FloatingPoint<Scalar>)
        {
            ...
            for (size_t k = 0; k < min_mn; ++k) {
                ...
                // v = x + sign(x[0])*||x||*e_1, but when x[0]=0, use +||x|| to avoid issues
                Matrix<Scalar, DynamicSize, DynamicSize> v = x;
                Scalar sign = (x(0, 0) >= Scalar(0)) ? Scalar(1) : Scalar(-1);
                v(0, 0) += sign * x_norm;
                ...
                if (v_norm > std::numeric_limits<Scalar>::epsilon()) {
                    ...
                    // This applies H_k from the right by updating each row of Q
                    for (size_t i = 0; i < m; ++i) {
                        Scalar Q_i_v = Scalar(0);
                        for (size_t j = 0; j < m - k; ++j) {
                            Q_i_v += Q(i, k + j) * v(j, 0);
                        }
                        for (size_t j = 0; j < m - k; ++j) {
                            Q(i, k + j) -= Scalar(2) * Q_i_v * v(j, 0);
                        }
                    }
                }
            }
        ...
        constexpr size_t qr_rank() const
            requires(FloatingPoint<Scalar>)
        {
            if (m_rows == 0 || m_cols == 0) {
                return 0;
            ...
            // Count non-zero rows in R (more robust for rectangular matrices)
            size_t rank_count = 0;

            for (size_t i = 0; i < R.rows(); ++i) {
                // Check if row i has any non-zero element
                bool row_nonzero = false;
                for (size_t j = i; j < R.cols(); ++j) { // Upper triangular, so start from i
                    if (std::abs(R(i, j)) > std::numeric_limits<Scalar>::epsilon()) {
                        row_nonzero = true;
                        break;
                    }
                }
        ...
        // Euclidean norm (L2 norm for vectors)
        constexpr Scalar euclidean_norm() const
        ...
        constexpr Scalar frobenius_norm() const
            requires(FloatingPoint<Scalar>)
        {
        ...
        template <Numeric InScalar, ptrdiff_t InRows, ptrdiff_t InCols>
            requires(CompatibleForAddition<Rows, Cols, InRows, InCols>)
        friend constexpr auto operator+(Matrix&& lhs, const Matrix<InScalar, InRows, InCols>& rhs) {
            lhs += rhs;
            return std::move(lhs);
        }
        ...
        template <Numeric InScalar, ptrdiff_t InRows, ptrdiff_t InCols>
            requires(CompatibleForAddition<Rows, Cols, InRows, InCols> and std::same_as<Scalar, InScalar>)
        friend constexpr auto operator+(const Matrix& lhs, Matrix<InScalar, InRows, InCols>&& rhs) {
            rhs += lhs;
            return std::move(rhs);
        }
        ...
        template <Numeric InScalar, ptrdiff_t InRows, ptrdiff_t InCols>
            requires(CompatibleForAddition<Rows, Cols, InRows, InCols>)
        friend constexpr auto operator+(Matrix&& lhs, Matrix<InScalar, InRows, InCols>&& rhs) {
            lhs += rhs;
            return std::move(lhs);
        }
    ...
    template <Numeric InScalar, ptrdiff_t InRows, ptrdiff_t InCols>
    std::ostream& operator<<(std::ostream& os, const Matrix<InScalar, InRows, InCols>& matrix) {
    ...
    constexpr void swap(Matrix<Scalar, Rows, Cols>& lhs, Matrix<Scalar, Rows, Cols>& rhs) noexcept {
        lhs.swap(rhs);
    }
