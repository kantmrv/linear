...
namespace linear {
    ...
    class Base_Solver {
      public:
        ...
        virtual Matrix<Scalar, DynamicSize, DynamicSize>
        solve(const Matrix<Scalar, DynamicSize, DynamicSize>& A,
              const Matrix<Scalar, DynamicSize, DynamicSize>& b) const = 0;
    };
    ...
    class Iter_Solver : public Base_Solver<Scalar> {
      ...
      public:
        explicit Iter_Solver(size_t max_iter = 1e10, Scalar tolerance = std::numeric_limits<Scalar>::epsilon())
        ...
        solve(const Matrix<Scalar, DynamicSize, DynamicSize>& A,
              ...
              const Matrix<Scalar, DynamicSize, DynamicSize>& x0) const = 0;

        // Solve with zero initial guess (override from Base_Solver)
        Matrix<Scalar, DynamicSize, DynamicSize>
        solve(const Matrix<Scalar, DynamicSize, DynamicSize>& A,
              const Matrix<Scalar, DynamicSize, DynamicSize>& b) const override {
        ...
        constexpr Scalar prior_estimate(const Matrix<Scalar, DynamicSize, DynamicSize>& A, size_t iterations) const {
            ...
            return std::pow(max_ratio, static_cast<Scalar>(iterations)) / (Scalar(1) - max_ratio);
        }

        // A posteriori error estimate: ||e_k|| ≤ (||M|| / (1 - ||M||)) * ||r_k||
        // where r_k = x_k - x_{k-1} is the residual between iterations
        constexpr Scalar posterior_estimate(const Matrix<Scalar, DynamicSize, DynamicSize>& A,
                                            const Matrix<Scalar, DynamicSize, DynamicSize>& x_curr,
                                            const Matrix<Scalar, DynamicSize, DynamicSize>& x_prev) const {
            assert(A.rows() == A.cols());
            const size_t n = A.rows();
        ...
        constexpr void set_max_iterations(size_t max_iter) noexcept { m_max_iterations = max_iter; }

        constexpr void set_tolerance(Scalar tolerance) noexcept { m_tolerance = tolerance; }
    };
    ...
    class Jacobian_Solver : public Iter_Solver<Scalar> {
      public:
        ...
        solve(const Matrix<Scalar, DynamicSize, DynamicSize>& A,
              const Matrix<Scalar, DynamicSize, DynamicSize>& b,
              const Matrix<Scalar, DynamicSize, DynamicSize>& x0) const override {
            assert(A.rows() == A.cols());
            assert(b.rows() == A.rows());
    ...
    class Gauss_Seidel_Solver : public Iter_Solver<Scalar> {
      public:
        ...
        // x^{(k+1)}_i = (b_i - sum_{j<i} a_{ij} * x^{(k+1)}_j - sum_{j>i} a_{ij} *
        // x^{(k)}_j) / a_{ii}
        Matrix<Scalar, DynamicSize, DynamicSize>
        solve(const Matrix<Scalar, DynamicSize, DynamicSize>& A,
              const Matrix<Scalar, DynamicSize, DynamicSize>& b,
